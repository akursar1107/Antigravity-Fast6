"""
Performance Breakdown Tab - Compare users across multiple dimensions.
"""
...
def show_performance_breakdown_tab(season: int) -> None:
    ...
    for user in users:
        try:
            ...
            st.warning(f"Error loading stats for {user['name']}: {e}")
    
    if not user_summaries:
        st.info("No performance data available for any users.")
        return
    
    user_df = pd.DataFrame(user_summaries)
    
    # Tabs for different comparison views
    tab1, tab2, tab3 = st.tabs(["Win Rate Comparison", "Brier Score Rankings", "Pick Volume"])
