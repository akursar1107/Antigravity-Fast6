...
from backend.utils.types import User, Week, Pick, Result, KickoffDecision, MarketOdds
from backend.utils.error_handling import log_exception
from backend.utils.caching import invalidate_on_pick_change, invalidate_on_result_change, invalidate_on_grading_complete
...
class MarketOddsRepository(BaseRepository):
    ...
    def find_for_game(self, game_id: str, source: Optional[str] = None) -> List[MarketOdds]:
        ...
        if source:
            ...
            query = f"SELECT * FROM {self.table_name} WHERE game_id = ? ORDER BY source, snapshot_time DESC"
            with get_db_context() as conn:
    ...
    def find_for_player(self, player_name: str, season: int, 
        ...
        if week:
            ...
            query = f"""
                ...
                ORDER BY week DESC, snapshot_time DESC
            """
    ...
    def find_for_week(self, season: int, week: int, 
        ...
        if source:
            results = self.find_where(
                ...
                order_by='game_id, player_name, snapshot_time DESC'
            )
        else:
            query = f"""
                SELECT * FROM {self.table_name}
    ...
    def create(self, game_id: str, player_name: str, source: str,
               implied_probability: float, snapshot_time: datetime,
               season: int, week: int, team: Optional[str] = None,
               american_odds: Optional[int] = None, raw_price: Optional[float] = None,
               volume: Optional[float] = None, market_id: Optional[str] = None) -> int:
        """Create a new market odds record (schema matches migration v8)."""
        data = {
            'game_id': game_id,
            'player_name': player_name,
            'source': source,
        ...
        if american_odds is not None:
            data['american_odds'] = american_odds
