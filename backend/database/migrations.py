...
from .connection import get_db_connection as get_connection
...
def get_current_version(conn: sqlite3.Connection) -> int:
    """
    Get the current schema version from backend.database.
...
def migration_v11_add_picks_unique_constraint(conn: sqlite3.Connection) -> None:
    ...
    # Create new table with constraint (preserve ON DELETE CASCADE from v1)
    cursor.execute('''
...
def migration_v15_fix_picks_game_id_created_at_swap(conn: sqlite3.Connection) -> None:
    """
    Version 15: Fix swapped game_id and created_at in picks table.
    
    Migration v11 used INSERT SELECT * which copied by position. The pre-v11
    table had created_at before game_id; v11 picked game_id before created_at.
    This caused created_at to get game_id values (e.g. '2025_01_KC_LAC') and
    game_id to get datetime values. Swap them back.
    """
    cursor = conn.cursor()
    
    # Check if fix is needed: created_at that looks like game_id (e.g. 2025_01_KC_LAC)
    cursor.execute("SELECT COUNT(*) FROM picks WHERE created_at LIKE '%_%_%_%' AND created_at GLOB '[0-9][0-9][0-9][0-9]_*'")
    needs_fix = cursor.fetchone()[0] > 0
    
    if not needs_fix:
        logger.info("Migration v15: picks game_id/created_at already correct, skipping")
        return
    
    cursor.execute("""
        UPDATE picks
        SET game_id = created_at,
            created_at = game_id
        WHERE created_at LIKE '%_%_%_%' AND created_at GLOB '[0-9][0-9][0-9][0-9]_*'
    """)
    updated = cursor.rowcount
    conn.commit()
    logger.info(f"Applied migration v15: Fixed {updated} picks with swapped game_id/created_at")


def migration_v16_ensure_weeks_season_2025(conn: sqlite3.Connection) -> None:
    """
    Version 16: Ensure weeks 1-18 exist for season 2025.
    
    Populates the weeks table as the source of truth so the public Stubs page
    and weeks API return the full list. Updates existing weeks with picks to
    season 2025 so they appear in the API; adds any missing weeks.
    """
    cursor = conn.cursor()
    season = 2025

    # Update weeks that have picks to season 2025 (so they appear in API)
    cursor.execute(
        "UPDATE weeks SET season = ? WHERE id IN (SELECT DISTINCT week_id FROM picks) AND season != ?",
        (season, season),
    )
    updated = cursor.rowcount if cursor.rowcount else 0
    if updated > 0:
        logger.info(f"Migration v16: Updated {updated} weeks to season {season}")

    # Add any missing weeks 1-18 for 2025
    added = 0
    for week in range(1, 19):
        cursor.execute("SELECT 1 FROM weeks WHERE season = ? AND week = ?", (season, week))
        if not cursor.fetchone():
            cursor.execute(
                "INSERT INTO weeks (season, week, started_at, ended_at) VALUES (?, ?, NULL, NULL)",
                (season, week),
            )
            added += 1
    conn.commit()
    if added > 0:
        logger.info(f"Applied migration v16: Added {added} weeks for season {season}")
    else:
        logger.info("Migration v16: All weeks for season 2025 already exist, skipping")


def migration_v17_create_touchdowns_table(conn: sqlite3.Connection) -> None:
    """
    Version 17: Create touchdowns table for materialized TD scorers.
    Populated by sync; eliminates on-the-fly PBP load for matchup page.
    """
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='touchdowns'")
    if cursor.fetchone():
        logger.info("Migration v17: touchdowns table already exists, skipping")
        return
    cursor.execute('''
    ...
    ''')
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_touchdowns_game ON touchdowns(game_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_touchdowns_season ON touchdowns(season)")
    conn.commit()
    logger.info("Applied migration v17: Created touchdowns table")


def migration_v19_dedupe_weeks_and_add_playoffs(conn: sqlite3.Connection) -> None:
    """
    Version 19: Remove duplicate weeks (keep first per season, week) and add weeks 19-22 for season 2025.

    Fixes duplicate Week 1 and adds playoff weeks (19=Wild Card, 20=Divisional, 21=Conference, 22=Super Bowl).
    """
    cursor = conn.cursor()
    season = 2025

    # Find duplicates: (season, week) with more than one row
    cursor.execute("""
        SELECT season, week, MIN(id) as keeper_id
        FROM weeks
    ...
    deleted = 0
    for row in dupes:
        s, w, keeper_id = row[0], row[1], row[2]
        # Reassign picks from duplicate weeks to the keeper
        cursor.execute(
            "UPDATE picks SET week_id = ? WHERE week_id IN (SELECT id FROM weeks WHERE season = ? AND week = ? AND id != ?)",
            (keeper_id, s, w, keeper_id),
        )
        # Delete duplicate rows (keep the one with keeper_id)
        cursor.execute(
            "DELETE FROM weeks WHERE season = ? AND week = ? AND id != ?",
            (s, w, keeper_id),
        )
        deleted += cursor.rowcount
    if deleted > 0:
        logger.info(f"Migration v19: Removed {deleted} duplicate week row(s)")

    # Add playoff weeks 19-22 for 2025 if missing
    added = 0
    ...
    conn.commit()
    if added > 0:
        logger.info(f"Migration v19: Added weeks 19-22 for season {season} ({added} new)")
    else:
        logger.info("Migration v19: Weeks 19-22 already exist, skipping")


def migration_v18_soft_delete_and_sync_metadata(conn: sqlite3.Connection) -> None:
    """
    Version 18: Add deleted_at to games (soft-delete), create sync_metadata table.
    """
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(games)")
    columns = [row[1] for row in cursor.fetchall()]
    if 'deleted_at' not in columns:
        cursor.execute("ALTER TABLE games ADD COLUMN deleted_at TIMESTAMP DEFAULT NULL")
        conn.commit()
        logger.info("Applied migration v18: Added deleted_at to games")
    ...
    if not cursor.fetchone():
        ...
        ''')
        conn.commit()
        logger.info("Applied migration v18: Created sync_metadata table")
...
MIGRATIONS: Dict[int, tuple[Callable[[sqlite3.Connection], None], str]] = {
    ...
    15: (migration_v15_fix_picks_game_id_created_at_swap, "Fix swapped game_id/created_at in picks"),
    16: (migration_v16_ensure_weeks_season_2025, "Ensure weeks 1-18 for season 2025"),
    17: (migration_v17_create_touchdowns_table, "Create touchdowns table for materialized TD scorers"),
    18: (migration_v18_soft_delete_and_sync_metadata, "Add deleted_at and sync_metadata"),
    19: (migration_v19_dedupe_weeks_and_add_playoffs, "Dedupe weeks and add playoff weeks 19-22"),
}


def migration_v20_add_user_base_bet(conn: sqlite3.Connection) -> None:
    """Version 20: Add base_bet column to users for per-user ROI stake."""
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(users)")
    columns = [row[1] for row in cursor.fetchall()]
    if "base_bet" not in columns:
        cursor.execute("ALTER TABLE users ADD COLUMN base_bet REAL DEFAULT NULL")
        conn.commit()
        logger.info("Applied migration v20: Added base_bet column to users")
    else:
        logger.info("Migration v20: base_bet already exists, skipping")


MIGRATIONS[20] = (migration_v20_add_user_base_bet, "Add base_bet column to users")
