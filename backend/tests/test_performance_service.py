...
class TestPickerPerformanceService(unittest.TestCase):
    ...
    def setUp(self):
        """Initialize service for testing with mocked repo."""
        # Mock repo returning standard test data
        self.mock_picks = [
             {'id': 1, 'user_id': 999, 'week_id': 1, 'team': 'KC', 'player_name': 'Mahomes', 'odds': -150, 'is_correct': True, 'game_type': 'regular'},
             {'id': 2, 'user_id': 999, 'week_id': 2, 'team': 'SF', 'player_name': 'Purdy', 'odds': 200, 'is_correct': False, 'game_type': 'regular'},
             {'id': 3, 'user_id': 999, 'week_id': 3, 'team': 'BUF', 'player_name': 'Allen', 'odds': 100, 'is_correct': True, 'game_type': 'playoff'}
        ]
        
        self.mock_repo = unittest.mock.MagicMock(return_value=self.mock_picks)
        self.service = PickerPerformanceService(picks_repo=self.mock_repo)
