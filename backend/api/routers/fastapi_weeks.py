...
from backend.api.fastapi_models import WeekCreate, WeekResponse
...
def _to_int(val) -> int:
    """Convert SQLite value to int. Handles bytes (little-endian) from old schema."""
    if val is None:
        return 0
...
def _row_to_week(w: Tuple) -> WeekResponse:
    """Convert SQLite row to WeekResponse. Handles bytes->int (SQLite schema quirk)."""
    return WeekResponse(
        id=_to_int(w[0]),
        season=_to_int(w[1]),
        week=_to_int(w[2]),
...
async def list_weeks(
    ...
    return [_row_to_week(w) for w in weeks]
...
async def get_week_stats(
    ...
    total = _to_int(stats[0]) if stats[0] is not None else 0
    graded = _to_int(stats[1]) if stats[1] is not None else 0
    return {
        "week_id": week_id,
        "season": _to_int(week_info[0]) if week_info[0] is not None else None,
        "week": _to_int(week_info[1]) if week_info[1] is not None else None,
        "total_picks": total,
        "graded_picks": graded,
        "correct_picks": _to_int(stats[2]) if stats[2] is not None else 0,
        "pending_picks": total - graded,
    }
